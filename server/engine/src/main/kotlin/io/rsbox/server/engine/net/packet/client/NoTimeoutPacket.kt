...
@ClientPacket(opcode = 0, type = PacketType.FIXED, length = 0)
class NoTimeoutPacket : Packet {

    override fun handle(session: Session) { /* Do Nothing */ }

    companion object : Codec<NoTimeoutPacket> {
        override fun decode(session: Session, buf: JagByteBuf): NoTimeoutPacket {
            return NoTimeoutPacket()
        }
    }
}