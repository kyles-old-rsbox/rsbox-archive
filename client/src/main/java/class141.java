public class class141 implements class360 {
   public static class292 field1300;
   public static final class141 field1301 = new class141(4, 2);
   public static final class141 field1302 = new class141(0, 0);
   public static final class141 field1303 = new class141(3, 3);
   public static final class141 field1304 = new class141(2, 4);
   public static final class141 field1306 = new class141(1, 1);
   static int field1307;
   final int field1308;
   public final int field1305;

   class141(int var1, int var2) {
      this.field1305 = var1;
      this.field1308 = var2;
   }

   public int method6656() {
      return this.field1308;
   }

   static boolean method2474(int var0, int var1) {
      return var0 != 4 || var1 < 8;
   }

   static int method2477(int var0, class78 var1, boolean var2) {
      class300 var4 = var2 ? class138.field1284 : class153.field1414;
      if (1600 == var0) {
         class57.field404[++class387.field4452 - 1] = var4.field3481;
         return 1;
      } else if (var0 == 1601) {
         class57.field404[++class387.field4452 - 1] = var4.field3482;
         return 1;
      } else {
         class297 var8;
         if (var0 == 1602) {
            if (var4.field3462 == 12) {
               var8 = var4.method5869();
               if (var8 != null) {
                  class57.field405[++class126.field1220 - 1] = var8.method5747().method7021();
                  return 1;
               }
            }

            class57.field405[++class126.field1220 - 1] = var4.field3522;
            return 1;
         } else if (var0 == 1603) {
            class57.field404[++class387.field4452 - 1] = var4.field3483;
            return 1;
         } else if (1604 == var0) {
            class57.field404[++class387.field4452 - 1] = var4.field3492;
            return 1;
         } else if (1605 == var0) {
            class57.field404[++class387.field4452 - 1] = var4.field3562;
            return 1;
         } else if (1606 == var0) {
            class57.field404[++class387.field4452 - 1] = var4.field3601;
            return 1;
         } else if (1607 == var0) {
            class57.field404[++class387.field4452 - 1] = var4.field3514;
            return 1;
         } else if (var0 == 1608) {
            class57.field404[++class387.field4452 - 1] = var4.field3513;
            return 1;
         } else if (1609 == var0) {
            class57.field404[++class387.field4452 - 1] = var4.field3612;
            return 1;
         } else if (var0 == 1610) {
            class57.field404[++class387.field4452 - 1] = var4.field3588;
            return 1;
         } else if (1611 == var0) {
            class57.field404[++class387.field4452 - 1] = var4.field3485;
            return 1;
         } else if (1612 == var0) {
            class57.field404[++class387.field4452 - 1] = var4.field3495;
            return 1;
         } else if (1613 == var0) {
            class57.field404[++class387.field4452 - 1] = var4.field3490.method6656();
            return 1;
         } else if (var0 == 1614) {
            class57.field404[++class387.field4452 - 1] = var4.field3519 ? 1 : 0;
            return 1;
         } else {
            class302 var5;
            if (1617 == var0) {
               var5 = var4.method5946();
               class57.field404[++class387.field4452 - 1] = var5 != null ? -374813941 * var5.field3617 * -1249433437 : 0;
            }

            if (var0 == 1618) {
               var5 = var4.method5946();
               class57.field404[++class387.field4452 - 1] = var5 != null ? var5.field3619 * 482449691 * 971643155 : 0;
               return 1;
            } else if (var0 == 1619) {
               var8 = var4.method5869();
               class57.field405[++class126.field1220 - 1] = null != var8 ? var8.method5601().method7021() : "";
               return 1;
            } else if (var0 == 1620) {
               var5 = var4.method5946();
               class57.field404[++class387.field4452 - 1] = null != var5 ? var5.field3618 * 1084956993 * 942829249 : 0;
               return 1;
            } else if (var0 == 1621) {
               var8 = var4.method5869();
               class57.field404[++class387.field4452 - 1] = null != var8 ? var8.method5611() : 0;
               return 1;
            } else if (1622 == var0) {
               var8 = var4.method5869();
               class57.field404[++class387.field4452 - 1] = var8 != null ? var8.method5612() : 0;
               return 1;
            } else if (1623 == var0) {
               var8 = var4.method5869();
               class57.field404[++class387.field4452 - 1] = var8 != null ? var8.method5613() : 0;
               return 1;
            } else if (var0 == 1624) {
               var8 = var4.method5869();
               class57.field404[++class387.field4452 - 1] = null != var8 && var8.method5585() ? 1 : 0;
               return 1;
            } else if (1625 != var0) {
               if (var0 == 1626) {
                  var8 = var4.method5869();
                  class57.field405[++class126.field1220 - 1] = var8 != null ? var8.method5602().method7005() : "";
                  return 1;
               } else if (1627 == var0) {
                  var8 = var4.method5869();
                  int var6 = var8 != null ? var8.method5683() : 0;
                  int var7 = null != var8 ? var8.method5606() : 0;
                  class57.field404[++class387.field4452 - 1] = Math.min(var6, var7);
                  class57.field404[++class387.field4452 - 1] = Math.max(var6, var7);
                  return 1;
               } else if (1628 == var0) {
                  var8 = var4.method5869();
                  class57.field404[++class387.field4452 - 1] = var8 != null ? var8.method5606() : 0;
                  return 1;
               } else if (1629 == var0) {
                  var8 = var4.method5869();
                  class57.field404[++class387.field4452 - 1] = null != var8 ? var8.method5815() : 0;
                  return 1;
               } else if (1630 == var0) {
                  var8 = var4.method5869();
                  class57.field404[++class387.field4452 - 1] = var8 != null ? var8.method5614() : 0;
                  return 1;
               } else if (1631 == var0) {
                  var8 = var4.method5869();
                  class57.field404[++class387.field4452 - 1] = var8 != null ? var8.method5739() : 0;
                  return 1;
               } else if (var0 == 1632) {
                  var8 = var4.method5869();
                  class57.field404[++class387.field4452 - 1] = var8 != null ? var8.method5617() : 0;
                  return 1;
               } else {
                  class34 var9;
                  if (1633 == var0) {
                     var9 = var4.method5871();
                     class57.field404[class387.field4452 - 1] = var9 != null ? var9.method617(class57.field404[class387.field4452 - 1]) : 0;
                     return 1;
                  } else if (var0 == 1634) {
                     var9 = var4.method5871();
                     class57.field404[class387.field4452 - 1] = null != var9 ? var9.method618((char)class57.field404[class387.field4452 - 1]) : 0;
                     return 1;
                  } else {
                     return 2;
                  }
               }
            } else {
               var8 = var4.method5869();
               class57.field404[++class387.field4452 - 1] = var8 != null && var8.method5604() ? 1 : 0;
               return 1;
            }
         }
      }
   }

   static int method2478(int var0, class78 var1, boolean var2) {
      if (7463 == var0) {
         boolean var4 = class57.field404[--class387.field4452] == 1;
         client.method197(var4);
         return 1;
      } else {
         return 2;
      }
   }
}
